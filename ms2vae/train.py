...
OUT_PREFIX = '/home/mallabhi/StrucNN/ms2vae/output/'
...
def loss_function(recon_intens_3d, slices, images, bnum, mu , logvar):
    '''Loss for NN'''
    recon_images = torch.zeros_like(images)
    for i in range(BATCH_SIZE):
        # Applying Symmetrization : Friedel Symmetry
...
def train_network(input_intens, orientation, slices, save=False):
    ...
    for i in range(len(orientation)//BATCH_SIZE):
        ...
        loss, recon_slice, sym_intens = loss_function(output, slices, images, i, mu, logvar)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if save:
            ...
            logvar_list.append(logvar.detach().cpu().clone().numpy())
    if not save:
...
for epoch in np.arange(N_EPOCHS) + 1:
    if (epoch%10)==0:
        ...
        torch.save(model.module.state_dict(), OUT_PREFIX + 'ms2_10subset_vae_dict')

        with h5py.File(OUT_PREFIX + 'ms2_10k_subset_vae_data.h5', "w") as f:
            f['loss'] = train_loss
...
print('Training Done, Data Saved')
