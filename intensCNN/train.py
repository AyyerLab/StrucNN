...
def best_projection_slice(recon_intens_3d, slices, ind):
    ...
    return F.grid_sample(
        recon_intens_3d.view(1, 1, size, size, size),
        grid.view(1, grid.shape[0], grid.shape[0], 1, 3),
        mode="bilinear",
        padding_mode="zeros",
        align_corners=True,
    )[0][0][:, :].reshape(grid.shape[0], grid.shape[0])
...
def loss_function(recon_intens_3d, slices, images, bnum):
    ...
    for i in range(BATCH_SIZE):
        ...
        bs = best_projection_slice(
            torch.Tensor.permute(symarrth, (0, 3, 2, 1)), slices, bnum * BATCH_SIZE + i
        )
        recon_images[i] = torch.Tensor.permute(bs, (1, 0))
        # No Symmetrization
...
def run_network(
    ...
    for i in range(len(orientation) // BATCH_SIZE):
        ...
        images = torch.from_numpy(intens_batch).view(
            (BATCH_SIZE, 1) + input_intens.shape[1:]
        )
        ...
        if not train and save:
            recon_vol.append(
                sym_intens.detach().cpu().clone().numpy().reshape((VOL_SIZE,) * 3)
            )
...
all_intens, all_ori, all_objsize = preprocessing.load_multiple_data(
    ["data/1FFK.h5", "data/1Y69.h5"], N_INTENS
)
qx_d, qy_d, qz_d = preprocessing.get_detector("data/1FFK.h5::detector")
...
train_data, valid_data = preprocessing.split_data(
    all_intens, all_ori, all_objsize, SPLIT_RATIO
)
...
for epoch in range(N_EPOCHS):
    ...
    if epoch < N_EPOCHS - 1:
        ...
        v_loss, v_true_intens, v_pred_intens, v_objsize, v_recon_vol = run_network(
            **valid_data
        )
